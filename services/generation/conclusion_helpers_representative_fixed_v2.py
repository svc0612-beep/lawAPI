...
def select_representative_direct_basis(
    ...
    우선순위
    1. 직접효과/후속효과 조문이 명시적으로 참조하는 direct basis
    2. 질문의 action/object가 선택된 항과 직접 연결되는 정도
    3. 기존 direct_basis 순서

    중요:
    - conditional_sanction은 질문 행위 자체와 자동으로 동일시할 수
      없는 별도 위반 조항이므로 대표 direct basis 결정 근거에서 제외한다.
    ...
    # ========================================================
    # A. 직접효과 / 후속효과 조문이 참조하는 direct article
    #
    # conditional_sanction은 별도 위반 요건이므로 제외한다.
    # 예를 들어 과태료 조문 전체에는 많은 법 조문 참조가 들어갈 수
    # 있어서 대표 direct basis를 왜곡할 수 있다.
    # ========================================================
    ...
    if isinstance(
        ...
        for basis in consequence_basis:
            ...
            if category == DIRECT_CONSEQUENCE:

                reference_weight = 120.0


            elif category == DOWNSTREAM_CONSEQUENCE:
                ...
                # conditional_sanction 및 기타 분류는
                # 대표 direct basis 결정에 사용하지 않는다.
                continue
    ...
    for index, basis in enumerate(
        ...
        # ====================================================
        # B. 질문의 실제 법적 효과 흐름이 이 direct article을
        #    명시적으로 참조하면 가장 강하게 우선한다.
        # ====================================================
