...
# 24-1. 조건부 제재 출력
#
# 조건부 제재 조문에는 서로 다른 위반행위가 여러 호로
# 함께 들어 있는 경우가 많다.
#
# 따라서:
# - 직접 효과 조문과 실제로 연결되는 호가 있을 때만 출력
# - 연결되는 호가 없으면 상세 화면에서도 제재 조문을 숨김
#
# 예:
# - 신고의무 위반 -> 해당 신고의무 조문을 참조하는 과태료 호
# - 질문과 무관한 다른 과태료 호 -> 출력하지 않음
#
# 특정 법률명/조문번호 하드코딩은 하지 않는다.
# ============================================================
...
def build_sanction_basis_lines(
    ...
    # 직접 연결 대상이 없으면 조건부 제재를 억지로 표시하지 않는다.
